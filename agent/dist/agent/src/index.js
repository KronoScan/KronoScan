import { WORLD_ID_VERIFIED, COORDINATOR_WS_URL, PRIVATE_KEY, resolveServiceConfig, } from "./config.js";
...
async function main() {
    ...
    const sessionId = await openOnChainSession(svc.sellerAddress, BigInt(svc.pricePerRequest), BigInt(svc.depositAmount), WORLD_ID_VERIFIED);
    // Step 5: Register session with coordinator
    console.log("\n-- Registering session with coordinator --");
    const session = await coordinator.openSession(sessionId, svc.sellerAddress, svc.pricePerRequest, svc.depositAmount, WORLD_ID_VERIFIED, svc.ensName);
    ...
    const summary = await runAudit(paymentFetch, coordinator, session.sessionId, session.effectivePrice, undefined, svc.sellerApiUrl);
    // Step 7: Close session
    console.log("\n-- Closing session --");
    const closed = await coordinator.closeSession(session.sessionId);
    // Step 8: Print summary
    const txDisplay = closed.txHash.length > 20
    ...
    if (getPaymentMode() === "fallback") {
        console.warn("WARNING: Payments were simulated (fallback mode). No real USDC was spent.");
    }
