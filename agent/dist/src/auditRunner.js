...
import { parseSSEStream, isCategoryComplete, isFinding, } from "./sseParser.js";
import { getPaymentMode } from "./x402Client.js";
export async function runAudit(paymentFetch, coordinator, sessionId, _effectivePrice, contractSource, sellerApiUrl) {
    const results = [];
    let totalCost = 0n;
    const bySeverity = {
        CRITICAL: 0,
        ...
        try {
            ...
            for (const event of events) {
                if (isFinding(event)) {
                    ...
                    console.log(`  ${severityIcon(event.severity)} [${event.severity}] ${event.title} (line ${event.line})`);
            ...
            const categoryPrice = CATEGORY_PRICES[category] ?? "100";
            try {
                const update = await coordinator.recordPayment(sessionId, category, categoryPrice);
                totalCost += BigInt(categoryPrice);
                console.log(`  Paid ${categoryPrice} | Total: ${update.totalConsumed} | Remaining: ${update.requestsRemaining}`);
            }
            catch (err) {
                console.error(`  Warning: Failed to record payment:`, err instanceof Error ? err.message : err);
                totalCost += BigInt(categoryPrice);
            }
